...
/** How long (ms) to sample per-rAF while a streaming response grows the tail. */
const STREAM_WINDOW_MS = 8000;

/**
 * Minimum per-frame scrollHeight growth (px) that counts as a real streaming
 * "growth commit" worth asserting on. Sub-pixel/layout noise stays below this;
 * a paced token chunk (usePacedText) grows the tail by at least a text line.
 */
const MEANINGFUL_GROWTH_PX = 12;

/**
 * Max distance-from-bottom (px) permitted ON the very frame a streaming growth
 * commit lands, while pinned. The pre-paint snap (morphdom write -> onContentGrown
 * -> snapToBottomIfPinned, fired synchronously in the same task that grew the DOM)
 * corrects scrollTop BEFORE the browser paints the taller content, so the
 * growth-commit frame is already glued (distance ~0). If the correction is left to
 * the POST-paint ResizeObserver only (the pre-fix behavior), the growth-commit
 * frame paints at the stale scrollTop first — distance ~= the chunk height (>> this
 * bound) — and is corrected a frame later: the per-chunk flicker. 16px sits above
 * sub-pixel settle but far below a stale one-chunk gap.
 */
const STREAM_PINNED_EPSILON_PX = 16;

/** Chat composer input — textarea or contenteditable (ChatInput.tsx). */
const CHAT_INPUT = 'textarea[placeholder], [contenteditable="true"][data-chat-input]';
...
test.describe('Chat session-open scroll: instant bottom + no oscillation (WI-C)', () => {
    ...
    test('AC1 — a PINNED streaming response stays glued on the growth-commit frame (pre-paint snap, no per-chunk flicker)', async ({ page }) => {
        // Drives the REAL streaming render path end-to-end: a live agent response
        // grows the trailing markdown block via the morphdom async write, and the
        // pre-paint snap (onContentGrown -> notifyContentChange -> snapToBottomIfPinned,
        // fired synchronously in the same task that mutated the DOM) must correct
        // scrollTop BEFORE the browser paints the taller content. A per-rAF sampler
        // runs BEFORE paint each frame, so the frame a growth commit lands on reveals
        // whether the correction was pre-paint (distance ~0, fixed) or deferred to the
        // post-paint ResizeObserver (distance ~= the chunk height first, then snapped a
        // frame later — the per-chunk flicker this test locks down). NO internal mocks:
        // real useChatAutoFollow + real MarkdownRenderer morphdom write.
        await installScrollSampler(page);

        const ids = await discoverSessionIds(page);
        test.skip(
            ids.length === 0,
            'No sessions in the sidebar — cannot open a chat surface to stream into. NOT a pass: ' +
            'skipped to avoid a vacuous green. Re-run against an instance with sessions.',
        );

        // Open the first session that yields a visible chat surface + composer.
        let opened = false;
        for (const id of ids) {
            await page.goto(`${BASE_URL}/?session=${id}`, { waitUntil: 'domcontentloaded' });
            const appeared = await page
        ...
        test.skip(
            !opened,
            'No session exposed a chat scroll container + composer — cannot drive a streaming ' +
            'response. NOT a pass: skipped to avoid a vacuous green.',
        );

        // Pin to the bottom, then reset the sampler so we only inspect the stream window.
        ...
        // Submit a prompt that produces a long, steadily-growing streamed answer.
        const input = page.locator(CHAT_INPUT).first();
        await input.click();
        await input.fill(
            'Write a long, detailed, multi-paragraph explanation (at least 12 paragraphs) of how ' +
            'browser scroll anchoring, ResizeObserver, requestAnimationFrame, and pre-paint layout ' +
            'effects interact. Keep writing steadily so the text streams in gradually.',
        );
        await input.press('Enter');

        // Sample per-rAF for the streaming window.
        await page.waitForTimeout(STREAM_WINDOW_MS);
        const samples = await readScrollSamples(page);

        // A streaming "growth commit" frame: scrollHeight grew vs the previous frame by
        // a meaningful amount, AND the previous frame was pinned at the bottom (so
        // auto-follow was active and IS expected to keep the tail glued).
        const growthFrames = [] as Array<{ grewPx: number; dist: number; prevDist: number }>;
        for (let i = 1; i < samples.length; i += 1) {
            const a = samples[i - 1]!;
            const b = samples[i]!;
            const grewPx = b.scrollHeight - a.scrollHeight;
            const prevDist = distanceFromBottom(a);
            if (grewPx >= MEANINGFUL_GROWTH_PX && prevDist <= PINNED_DISTANCE_TOLERANCE_PX) {
                growthFrames.push({ grewPx, dist: distanceFromBottom(b), prevDist });
            }
        }

        // PRECONDITION: at least one pinned meaningful-growth streaming frame must have
        // been observed, else the pre-paint pinned behavior was never exercised.
        test.skip(
            growthFrames.length === 0,
            'No pinned streaming growth-commit frame was observed within the window (the backend ' +
            'did not stream a steadily-growing response while pinned) — the pre-paint snap could ' +
            'not be exercised. NOT a pass: skipped to avoid a vacuous green. Re-run against a ' +
            'backend that streams a long response.',
        );

        // Every pinned growth-commit frame must ALREADY be at the bottom on that same
        // frame — proving the snap landed pre-paint. The pre-fix (post-paint RO only)
        // behavior paints the grown content at the stale scrollTop first, so the worst
        // growth-commit frame would sit ~= a chunk height from the bottom.
        const worst = growthFrames.reduce(
            (acc, f) => (f.dist > acc.dist ? f : acc),
            growthFrames[0]!,
        );
        expect(
            worst.dist,
            [
                `REGRESSION: a pinned streaming growth commit painted at a stale scrollTop`,
                `(per-chunk flicker). worst distance-from-bottom on a growth-commit frame=${worst.dist}px`,
                `for a +${worst.grewPx}px growth (tolerance ${STREAM_PINNED_EPSILON_PX}px).`,
                `growth-commit frames observed=${growthFrames.length}.`,
                `A distance ~= the growth height means the snap fired only from the POST-paint`,
                `ResizeObserver — the pre-paint onContentGrown -> snapToBottomIfPinned seam is not`,
                `firing synchronously inside the morphdom write.`,
            ].join(' '),
