...
export const ChatContainer: React.FC<ChatContainerProps> = ({ autoOpenDraft = true, readOnly = false }) => {
    ...
    // Scoped blocking requests — subscribe to permissions/questions for the current session +
    // descendant subagent sessions only. Directory-less child events are recovered into this
    // directory store by the sync layer (openchamber-5ki.9), so no cross-directory aggregation.
