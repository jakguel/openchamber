...
// viz-global.js is a UMD bundle whose global branch runs deterministically ONLY as a classic
// <script> (it assigns (globalThis||self).Viz); plantuml.js (ESM) then reads that bare `Viz`
// global. Importing viz-global as an ESM module risks UMD-interop rewrites skipping the global
// branch differently in dev vs prod — the loader risk the C0 spike proved out. Loading it via a
// script tag keyed off `?url` is dev/prod-identical.
...
export function loadPlantUmlEngine(): Promise<PlantUmlEngine> {
    // The engine needs document/canvas/getBBox; it throws in Node and mis-sizes in jsdom.
